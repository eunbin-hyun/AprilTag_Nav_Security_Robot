...
# 상태 LED (GPIO.BOARD 물리 핀 번호)
# 터치 센서 없이 실행하는 테스트 파일이라 TOUCH_PIN 입력은 쓰지 않는다.
...
def start_sensing_without_touch():
    sensing_enabled.set()
    GPIO.output(LED_PIN, GPIO.HIGH)
    print("[테스트] 터치 센서 없이 ToF/RFID 센싱을 바로 시작합니다.")
...
def main():
    ...
    print("터치 센서 없이 실행 중입니다. 종료하려면 Ctrl+C\n")
