...
- `web_bridge`는 로봇 상태 송신을 구현했으며 서버 명령 수신 루프는 추가 작업이 필요합니다.
- BNO085 수신 경로는 구현했지만 실물 IMU는 최종 차량에 장착하지 못했습니다.
- 시뮬레이션에서 yaw 편향은 ±8°와 −12°를 통과했지만 +10°에서는 복귀 태그가 화각을 벗어났습니다.
- 사람 감지와 TTS는 프로토타입·평가 단계이며 주행 상시 프로세스에는 연결하지 않았습니다.
